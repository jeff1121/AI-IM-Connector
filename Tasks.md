...
## 第三階段：Copilot SDK 整合（ACP 客戶端）

- [x] **3.1** 安裝 `GitHub.Copilot.SDK` NuGet 套件 (v0.1.23)
- [x] **3.2** 研究 SDK API（CopilotClient、CopilotSession、SessionConfig、MessageOptions）
- [x] **3.3** 定義 `ICopilotClientService` 介面（啟動/停止、建立/恢復 Session、發送訊息）
- [x] **3.4** 實作 `CopilotClientService`（封裝 CopilotClient，管理 CLI 子行程生命週期）
- [x] **3.5** 實作 `CopilotSessionManager`（使用者 Session 快取 + SDK 原生 persist/resume）
- [x] **3.6** 更新 Configuration 模型（CliPath、ResponseTimeoutSeconds、Model）
- [x] **3.7** 實作 `CopilotLifecycleService`（BackgroundService，隨應用程式啟停 CLI）
...
|------|------|------|
| .NET | 8.0 | WebAPI 框架 |
| GitHub.Copilot.SDK | 0.1.23 | Copilot CLI 封裝，ACP (stdio) 通訊 |
| LineBotSDK | — | LINE Messaging API |
| Telegram.Bot | — | Telegram Bot API |
| Docker | — | 多階段建置容器化部署 |
| xUnit + Moq | — | 單元測試框架 |
...
| 2026-02-11 | 清理舊 AcpMessages 模型、更新 MediaHandler、全面更新文件 |
